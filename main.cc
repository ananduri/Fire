...
int main(int argc, char *argv[]) {
  ...
  for (int timestep = 0; timestep < 5; ++timestep) {
    render_particles(particles, quadtree, WIDTH, HEIGHT, use_quadtree, timestep);
    step_particles(particles);
  }
  
  // Run post-processing step to create the MP4? Or do it in CLI?
  std::system("ffmpeg -r 10 -i /Users/drake/Documents/projects/genart/fire_cpp/output/img_%d.ppm -c:v libx264 -crf 25 -vf 'scale=500:500,format=yuv420p' -movflags +faststart /Users/drake/Documents/projects/genart/fire_cpp/output/output.mp4");
}
