...
void render_particles(std::vector<Particle*> const& particles, QuadTree const& quadtree, int const width, int const height, int const index, bool const use_quadtree) {
  std::ofstream file_handle;
  //file_handle.open(std::format("output/img_{}", index));
  // Do I have to use the abolute path?
  file_handle.open("/Users/drake/Documents/projects/genart/fire_cpp/output/img_0.ppm");
